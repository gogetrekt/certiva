...
interface BulkDeleteDocumentProofsModalProps {
  selectedIds: string[];
  onClose: () => void;
  onSuccess: (result: { deleted: number; skipped: number; failed: number }) => void;
}

export function BulkDeleteDocumentProofsModal({
  selectedIds,
  onClose,
  onSuccess,
}: BulkDeleteDocumentProofsModalProps) {
  const { t } = useLanguage();
  const titleId = useId();
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const response = await fetch("/api/document-proofs/bulk-delete", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(body.message ?? t.forms.bulkDeleteDocumentProofs.unable);
      }
      const result = (await response.json()) as { deleted: number; skipped: number; failed: number };
      onSuccess(result);
    } catch (err) {
      setError(err instanceof Error ? err.message : t.forms.bulkDeleteDocumentProofs.unable);
    } finally {
      ...
      >
        <div className="border-b border-[hsl(var(--border-default))] px-6 py-5">
          <p id={titleId} className="text-sm font-semibold text-[hsl(var(--text-primary))]">
            {t.forms.bulkDeleteDocumentProofs.title}
          </p>
          <p id={descriptionId} className="mt-0.5 text-xs text-[hsl(var(--text-tertiary))]">
            {t.forms.bulkDeleteDocumentProofs.description}
          </p>
        </div>
        ...
        <div className="space-y-4 px-6 py-5">
          <div className="rounded-lg border border-[hsl(var(--border-default))] bg-[hsl(var(--bg-subtle))] px-4 py-3">
            <p className="kicker mb-1">
              {selectedIds.length} {t.forms.bulkDeleteDocumentProofs.selectedCount}
            </p>
          </div>
          ...
          <div className="rounded-lg border border-[hsl(var(--status-error-border))] bg-[hsl(var(--status-error-bg))] px-4 py-3 text-xs leading-5 text-[hsl(var(--status-error-text))]">
            {t.forms.bulkDeleteDocumentProofs.warning}
          </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-[hsl(var(--border-default))] px-6 py-4">
          ...
          >
            {isSubmitting ? t.forms.bulkDeleteDocumentProofs.deleting : t.forms.bulkDeleteDocumentProofs.confirm}
          </button>
        </div>
      </div>
    </div>
